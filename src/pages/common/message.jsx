...
const Message = () => {
  ...
  return (
    <>
        ...
        <a className="icon-link text-decoration-none text-black">
          ...
          {JSON.parse(localStorage.getItem("user")).role == "bidder" && (
            <Link
              ...
              to={`/userpage/supplier/${id}/messages`}
            >
          ...
          {JSON.parse(localStorage.getItem("user")).role ==
            ...
            <Link
              className="text-decoration-none"
              to={`/userpage/pendch/${id}/messages`}
            >
              Back to All Messages
            </Link>
          )}
      ...
      <div
        className="border mb-5 container rounded"
      ...
      >
        ...
        {isFetching ? (
          ...
          <div className="mx-auto text-danger" style={{ width: "5rem" }}>
            Error Fetching!
          </div>
        ) : (
          <div className="mx-auto container" style={{ minHeight: "10rem" }}>
            <ul className="list-unstyled">
              <li>
                {message.from == JSON.parse(localStorage.getItem("user")).email
                  ? "To :"
                  : "From :"}{" "}
                <p className="m-0 d-inline fw-bold">{message.from_name}</p>
              </li>
              <li className="mt-2">
                Subject :{" "}
                <p className="m-0 d-inline fw-bold">{message.subject}</p>
              </li>
              <li className="mt-2">
                Message :{" "}
                <div
                  className="border container p-2"
                  style={{ maxHeight: "5rem", overflow: "auto" }}
                >
                  <p
                    className="m-0 d-inline text-break"
              ...
              {message.file && (
                <li className="mt-2">
                  Attached File :{" "}
                  <div
                    className="d-inline-block container p-2"
                    style={{ maxHeight: "5rem", overflow: "auto" }}
                  >
                    <a
                      href={`/backend/uploads/msgfiles/${message.file}`}
                      download
                    >
      ...
      {JSON.parse(localStorage.getItem("user")).role !== "ppa it officer" && (
        <Footer />
      )}
    </>
  );
