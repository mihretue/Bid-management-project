...
const fetchProcHead=(ent)=>{
  ...
  .then((res)=>{
    ...
    if(localStorage.getItem('user')){
       checkParticipationStatus()}else{setIsFetching(false)}
...
const checkParticipationStatus=()=>{
    let q={bidderId:JSON.parse(localStorage.getItem('user')).id,bidId:tender._id}
    q=new URLSearchParams(q)
...
<ul>
    <li>Procuring Entity : <p className="m-0 d-inline fw-bold">{depHead.pBody}</p></li>
...
<p className="m-0">{participated==false&&"You didn't participate on this tender."}</p>
</div>}<hr className="w-100" />
<button className="btn btn-primary mt-2">
        <a download className="text-white text-decoration-none" href={`/backend/uploads/bidrequests/${tender.bidAwardFile}`}>View Bid Award</a>
    </button>
</div>)}
<Footer />
</>)
