...
const BidderBidsDetail = () => {
  ...
  return (
    <>
      <div className="container mb-4">
        <a className="icon-link text-decoration-none text-black">
          ...
          <Link
            ...
            to={`/userpage/supplier/${id}/all-bids`}
          >
            Back to All Bids
          </Link>
        </a>
      ...
      <div
        className="py-2 container border rounded d-flex flex-column align-items-center justify-content-center mx-auto mb-5"
          ...
          {isFetching ? (
            ...
            <>
              <ul className="">
                ...
                <li>
                  Procuring Entity :{" "}
                  <p className="m-0 d-inline fw-bold">{tender.ent}</p>
                </li>
                <li>
                  Bid Opening Schedule :{" "}
                  <p className="m-0 d-inline fw-bold">{tender.open}</p>
                </li>
                <li>
                  Site Visitation Schedule :{" "}
                  <p className="m-0 d-inline fw-bold">{tender.visit}</p>
                </li>
                <li className="mt-2">
                  Bid Status :{" "}
                  <p
                    className={
                      tender.open >= Date.now()
                        ? "m-0 d-inline font-monospace text-primary fw-bold"
                        : "m-0 d-inline fw-bold"
                    }
                  >
                    {tender.open >= Date.now() ? "Open" : "Not Open"}
                  </p>
                </li>
              </ul>
              <button
                onClick={() => {
                  navigate(`/tenders/${bid}`);
                }}
          ...
          {bIsFetching ? (
            ...
            <>
              <ul className="">
                <li>
                  Applied On:{" "}
                  <p className="m-0 d-inline fw-bold">{bidding.appTime}</p>
                </li>
                <li>
                  Bid Document Payment :{" "}
                  <p className="m-0 d-inline fw-bold">
                    {bidding.bidDocPayment}
                  </p>
                </li>
                <li>
                  Bid Security Payment :{" "}
                  <p className="m-0 d-inline fw-bold">
                    {bidding.bidSecPayment}
                  </p>
                </li>
                <li>
                  Bid Proposal Document :
                  <a
                    href={`/backend/uploads/biddocs/${bidding.bidPropFile}`}
                    download
              ...
              {bidding.status == "cancelled" ? (
                <h4 className="text-danger mt-3">
                  You Have Withdrawn From This Tender!
                </h4>
                ...
                <Pane className="mx-auto mt-2 d-flex justify-content-center">
                  ...
                  >
                    <p className="m-0 fw-bold">
                      Are You Sure You Want To Withdraw From This Tender?
                    </p>
                    <p className="mx-0 mt-2" style={{ fontSize: "0.8rem" }}>
                      Regarding Bid Security Payment, Bid Security Payments
                      aren't to be returned for withdrawing bidders.
                    </p>
                  </Dialog>
                  <button
                    id="del_btn"
