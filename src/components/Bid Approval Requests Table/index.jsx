...
import {AiOutlineSearch} from 'react-icons/ai'
  ...
  export default function StickyHeadTable() {
    ...
    const fetchBids=()=>{
      let q={pBody:JSON.parse(localStorage.getItem('user')).pBody,approvalRequested:true}
      q=new URLSearchParams(q)
      fetch(`http://localhost:3001/tenderbyprop?${q}`)
      .then(res=>res.json())
      .then((res)=>{
        setBids(res)
        setInit(res)
        ...
        :(bids.length==0?
          ...
          <TextField
            ...
            onChange={(e)=>{
             ...
             let filt=bids;
            if(q!=""){
               filt=bids.filter(bid=>(bid.fName+" "+bid.lName).includes(q))
               setBids(filt)
            }
             else 
               setBids(init)
